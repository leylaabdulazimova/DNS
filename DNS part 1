...
# Define a function to query the local DNS server for the IP address
...
# Define a function to query a public DNS server for the IP address
# of a given domain name
...
# Define a function to compare the results from the local and public
# DNS servers for each domain name in the list
def compare_dns_servers(domainList, question_type):
    for domain_name in domainList:
        local_ip_address = query_local_dns_server(
            domain_name,
            question_type
        )
        public_ip_address = query_dns_server(
            domain_name,
            question_type
        )
...
# Define a function to print the results from querying both the local
# and public DNS servers for each domain name in the domainList
def local_external_DNS_output(question_type):
    print("Local DNS Server")

    for domain_name in domainList:
        ip_address = query_local_dns_server(
            domain_name,
            question_type
        )
...
if __name__ == '__main__':
    ...
    # Call the function to compare the results from both DNS servers
